...
namespace ComPact.ConsumerTests
{
    ...
    public class PactV3ConsumerTests
    {
        ...
        [TestMethod]
        public async Task ShouldMatchPutRequest()
        {
            var url = "http://localhost:9396";
            ...
            receiptObject.Add("amount", 5.5);
            receiptObject.Add("unit", "gram");
            ...
            using (var client = new HttpClient { BaseAddress = new Uri(url) })
            {
                StringContent httpContent = new StringContent(receiptObject.ToString(), Encoding.UTF8, "application/json");
                client.DefaultRequestHeaders.Add("Accept", "application/json");
                var response = await client.PutAsync($"api/recipes/{recipeId}", httpContent);
                Assert.IsTrue(response.IsSuccessStatusCode);
            }
        ...
        [TestMethod]
        public async Task ShouldMatchPostRequest()
        {
            var url = "http://localhost:9396";
