...
namespace ComPact.Models.V2
{
    public class Response
    {
        ...
        private Dictionary<JToken, Matcher> LinkMatchingRulesToTokens(JToken rootToken)
        {
            var orderedPaths = MatchingRules.Select(m => m.Key).OrderBy(m => m, new MatchingRulePathComparer()).ToList();

            var applicableMatchingRules = new Dictionary<JToken, Matcher>();
        ...
        private void CompareTokenAndItsChildren(JToken token, Dictionary<JToken, Matcher> applicableMatchingRules, JObject actualJObject, List<string> differences)
        {
        ...
        private string CompareExpectedTokenWithActual<T>(JToken expectedToken, Dictionary<JToken, Matcher> applicableMatchingRules, JObject actualJObject) where T: IEquatable<T>
        {
        ...
        private string CompareExpectedArrayWithActual(JToken expectedToken, Dictionary<JToken, Matcher> applicableMatchingRules, JObject actualJObject)
        {
            var actualToken = actualJObject.SelectToken(expectedToken.Path);
            if (actualToken == null)
            {
        ...
        private Matcher GetMatchingRuleForToken(JToken token, Dictionary<JToken, Matcher> applicableMatchingRules)
        {
